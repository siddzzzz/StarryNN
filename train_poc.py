import torch
import numpy as np
from tasks import load_tabular_data, get_tabular_batches
...
def train():
    ...
    X_train, Y_train, X_test, Y_test = load_tabular_data()
    ...
    # Initialize network
    # We will map the 30 input features to nodes 0-29.
    # Exit node will automatically be Node N-1 (Node 63).
    net = StarryNet(
    ...
    with torch.no_grad():
        for epoch in range(1, epochs + 1):
            ...
            for X_batch, Y_batch in get_tabular_batches(X_train, Y_train, batch_size=batch_size, shuffle=True):
                ...
                # 2. Clamped Phase: inputs settle while exit node is nudged towards targets
            ...
            if epoch % 20 == 0 or epoch == 1:
                ...
                loss_train = torch.mean((p_train - Y_train.squeeze(-1)) ** 2).item()
                acc_train = ((p_train > 0.0) == (Y_train.squeeze(-1) > 0.0)).float().mean().item()
